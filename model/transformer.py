...
class ScaledPositionalEncoding(PositionalEncoding):
    ...
    def forward(self, x):
        seq_len = x.size(1)
        x = x + self.alpha * self.pe[:, :seq_len, :]
        return self.dropout(x)
...
class TransformerEncoder(nn.Module):  # the transformer encoder modified for temporal data
    ...
    def __init__(
        ...
        assert pe_method in (None, "origin", "scale",), \
            "Invalid pe method: {}".format(pe_method)
        ...
        # )
        self.mlp_head = []
        self.mlp_head.append(nn.LayerNorm(dim))
...
def build_transformer_from_config(config, input_dim):
    """Factory to build TransformerEncoder from a global config and input feature dim.
    """
    
    transformer_cfg = getattr(config, 'transformer', {}) or {}
    
    return TransformerEncoder(
        num_classes=1,
        dim=transformer_cfg.get('dim', input_dim),
        depth=transformer_cfg.get('depth', 2),
        heads=transformer_cfg.get('heads', 8),
        ff_dim=transformer_cfg.get('ff_dim', 256),
        mlp_head_hidden_dim=transformer_cfg.get('mlp_head_hidden_dim', None),
        use_class_token=transformer_cfg.get('use_class_token', True),
        pool=transformer_cfg.get('pool', 'cls'),
        dim_head=transformer_cfg.get('dim_head', 64),
        dropout=transformer_cfg.get('dropout', 0.0),
        mlp_head_dropout=transformer_cfg.get('mlp_head_dropout', 0.0),
        pe_method=transformer_cfg.get('pe_method', None),
        pe_max_len=transformer_cfg.get('pe_max_len', 5000),
        activation=transformer_cfg.get('activation', 'gelu'),
        sigmoid=transformer_cfg.get('sigmoid', True),
        dp_enable=transformer_cfg.get('dp_enable', False),
    )